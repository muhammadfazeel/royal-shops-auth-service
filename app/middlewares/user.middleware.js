...
const validateSignUp = (req, res, done) => {
  ...
  if (_.isEmpty(body.address) || !_.isString(body.address) || body.address.length < 2 || body.address.length > 300) {
    errorArray.push({
      field: 'address',
      error: 10040,
      message: '\'address\' is required as string, length must be between 2 and 100.'
    })
  }

  // phone is required, validating it as not empty, valid String and length range.
  if (_.isEmpty(body.phone) || !_.isString(body.phone) || body.phone.length < 2 || body.phone.length > 15) {
    errorArray.push({
      field: 'phone',
      error: 10020,
      message: '\'phone\' is required as string, length must be between 2 and 100.'
    })
  }
  ...
  if (_.isEmpty(body.nationalId) || !_.isString(body.nationalId) || body.nationalId.length < 2 || body.nationalId.length > 15) {
    errorArray.push({
      field: 'nationalId',
      error: 10020,
      message: '\'nationalId\' is required as string, length must be between 2 and 100.'
    })
  }
...
const validateEmailVerification = (req, res, done) => {
  ...
  if (!_.isEmpty(errorArray)) {
    return generalMiddleware.standardErrorResponse(res, errorArray, 'user.middleware.validateEmailVerification')
  }
